...
describe("Page Functionality Tests", () => {
  ...
  it("Product Detail Page: loads product and adds to cart", async () => {
    ...
    expect(mockCart.addItem).toHaveBeenCalledWith(expect.objectContaining({
      id: 1,
      name: "Detailed Product",
    }));
  });
  ...
  it("Cart Page: displays items and summary", async () => {
    mockCart.items = [
      { id: 1, name: "Cart Item 1", price: 50, qty: 2, image: "/img1.jpg" },
    ];
  ...
  it("Checkout Page: renders form", async () => {
    ...
    expect(screen.getByText("Shipping Information")).toBeInTheDocument(); // Assuming this text exists in CheckoutForm
  });
  ...
  it("Account Page: redirects to login if not authenticated", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Sign In")).toBeInTheDocument(); // Should show LoginForm
    });
  });
  ...
  it("Account Page: loads user data if authenticated", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue("Test User")).toBeInTheDocument(); // First name input
