...
router.post("/register", authLimiter, validators, async (req, res) => {
  ...
  try {
    ...
    await sendEmail({ to: email, subject: "Verify your email", template: "order_processing", data: { orderId: rows[0].id, eta: "Verify" } });
    res.json({ ok: true });
  } finally {
    client.release();
  }
...
router.post("/login", authLimiter, validators, async (req, res) => {
  ...
  const { rows } = await pool.query("SELECT id,password_hash,is_verified FROM users WHERE email=$1", [email]);
  ...
  res.cookie("refresh_token", refresh, { httpOnly: true, secure: true, sameSite: "strict", maxAge: 7 * 24 * 3600 * 1000 });
  res.json({ access });
});

router.post("/verify", authLimiter, body("token").isString(), async (req, res) => {
  const { token } = req.body;
  const { rowCount } = await pool.query("UPDATE users SET is_verified=true, verify_token=NULL WHERE verify_token=$1", [token]);
...
router.post("/forgot", authLimiter, body("email").isEmail(), async (req, res) => {
  ...
  const token = uuidv4();
  await pool.query("UPDATE users SET reset_token=$1, reset_requested_at=now() WHERE id=$2", [token, rows[0].id]);
  await sendEmail({ to: email, subject: "Reset password", template: "order_processing", data: { orderId: token, eta: "Reset" } });
  res.json({ ok: true });
});

router.post("/reset", authLimiter, [body("token").isString(), body("password").isLength({ min: 8 })], async (req, res) => {
  ...
  const hash = await argon2.hash(password);
  await pool.query("UPDATE users SET password_hash=$1, reset_token=NULL WHERE id=$2", [hash, rows[0].id]);
  res.json({ ok: true });
});
